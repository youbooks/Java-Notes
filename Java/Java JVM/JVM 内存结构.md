...
![2021-01-06-R1R96Y](https://image.ldbmcs.com/2021-01-06-R1R96Y.jpg)
...
![2021-01-06-rZmfO9](https://image.ldbmcs.com/2021-01-06-rZmfO9.jpg)
...
- 如果采用固定大小的 Java 虚拟机栈，那每个线程的 Java 虚拟机栈容量可以在线程创建的时候独立选定。如果线程请求分配的栈容量超过 Java 虚拟机栈允许的最大容量，Java 虚拟机将会抛出一个 **StackOverflowError** 异常。dd
...
![2021-01-06-XhKWTX](https://image.ldbmcs.com/2021-01-06-XhKWTX.jpg)
...
![2021-01-06-UEYwcL](https://image.ldbmcs.com/2021-01-06-UEYwcL.jpg)
...
- **栈帧中的局部变量表中的槽位是可以重用的**，如果一个局部变量过了其作用域，那么在其作用域之后申明的新的局部变量就很有可能会复用过期局部变量的槽位，从而**达到节省资源的目的**。（下图中，this、a、b、c 理论上应该有 4 个变量，c 复用了 b 的槽）。
  ...
  ![2021-01-06-WdV6Rs](https://image.ldbmcs.com/2021-01-06-WdV6Rs.jpg)
...
- 在 Java 源文件被编译到字节码文件中时，所有的变量和方法引用都作为**符号引用**（Symbolic Reference）保存在 Class 文件的常量池中。比如：描述一个方法调用了另外的其他方法时，就是通过常量池中指向方法的符号引用来表示的，那么**动态链接的作用就是为了将这些符号引用转换为调用方法的直接引用**。
  ...
  ![2021-01-06-cBsTNh](https://image.ldbmcs.com/2021-01-06-cBsTNh.jpg)
...
![2021-01-06-8W32TY](https://image.ldbmcs.com/2021-01-06-8W32TY.jpg)
...
![2021-01-06-nifuOA](https://image.ldbmcs.com/2021-01-06-nifuOA.jpg)
...
#### 4.6.1 什么是 TLAB （Thread Local Allocation Buffer）?
...
`StringBuffer sb`是一个方法内部变量，上述代码中直接将sb返回，这样这个 StringBuffer 有可能被其他方法所改变，这样它的作用域就不只是在方法内部，虽然它是一个局部变量，称其逃逸到了方法外部。甚至还有可能被外部线程访问到，譬如赋值给类变量或可以在其他线程中访问的实例变量，称为**线程逃逸**。
...
- **方法区（method area）只是JVM 规范中定义的一个概念**，用于存储类信息、常量池、静态变量、JIT编译后的代码等数据，并没有规定如何去实现它，不同的厂商有不同的实现。而**永久代（PermGen）是 虚拟机特有的概念， Java8 的时候又被元空间**取代了，永久代和元空间都可以理解为方法区的落地实现。
...
![2021-01-06-1jcuSi](https://image.ldbmcs.com/2021-01-06-1jcuSi.jpg)
...
![2021-01-06-B5afWE](https://image.ldbmcs.com/2021-01-06-B5afWE.jpg)
...
![2021-01-06-D6kgEM](https://image.ldbmcs.com/2021-01-06-D6kgEM.jpg)
