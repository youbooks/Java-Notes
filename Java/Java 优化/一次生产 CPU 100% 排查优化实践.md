> 转载：[一次生产 CPU 100% 排查优化实践](https://crossoverjie.top/2018/12/17/troubleshoot/cpu-percent-100/)
...
![2020-11-08-PzasBh](https://image.ldbmcs.com/2020-11-08-PzasBh.jpg)
...
```bash
cat pid.log | grep '2f6eb' -C 8
```

![2020-11-08-rvEt6c](https://image.ldbmcs.com/2020-11-08-rvEt6c.jpg)

发现这是 `Disruptor` 的一个堆栈，前段时间正好解决过一个由于 Disruptor 队列引起的一次 [OOM](https://crossoverjie.top/2018/12/17/troubleshoot/cpu-percent-100/)：[强如 Disruptor 也发生内存溢出？](https://crossoverjie.top/2018/08/29/java-senior/OOM-Disruptor/)
...
![2020-11-08-ZW7lwG](https://image.ldbmcs.com/2020-11-08-ZW7lwG.jpg)
...
![2020-11-08-ckBTm9](https://image.ldbmcs.com/2020-11-08-ckBTm9.jpg)
...
![2020-11-08-uWqrtW](https://image.ldbmcs.com/2020-11-08-uWqrtW.jpg)

![2020-11-08-sxyt7q](https://image.ldbmcs.com/2020-11-08-sxyt7q.jpg)

创建了 15 个 `Disruptor` 队列，同时每个队列都用线程池来往 `Disruptor队列` 里面发送 100W 条数据。

消费程序仅仅只是打印一下。

![2020-11-08-quC565](https://image.ldbmcs.com/2020-11-08-quC565.jpg)

跑了一段时间发现 CPU 使用率确实很高。

![2020-11-08-rrlLql](https://image.ldbmcs.com/2020-11-08-rrlLql.jpg)

同时 `dump` 线程发现和生产的现象也是一致的：消费线程都处于 `RUNNABLE` 状态，同时都在执行 `yield`。

通过查询 `Disruptor` 官方文档发现：

![2020-11-08-l372HR](https://image.ldbmcs.com/2020-11-08-l372HR.jpg)

> YieldingWaitStrategy 是一种充分压榨 CPU 的策略，使用`自旋 + yield`的方式来提高性能。
> 当消费线程（Event Handler threads）的数量小于 CPU 核心数时推荐使用该策略。

![2020-11-08-jw5LmF](https://image.ldbmcs.com/2020-11-08-jw5LmF.jpg)

同时查阅到其他的等待策略 `BlockingWaitStrategy` （也是默认的策略），它使用的是锁的机制，对 CPU 的使用率不高。

于是在和之前同样的条件下将等待策略换为 `BlockingWaitStrategy`。

![2020-11-08-STb8jQ](https://image.ldbmcs.com/2020-11-08-STb8jQ.jpg)

![2020-11-08-0sLqnj](https://image.ldbmcs.com/2020-11-08-0sLqnj.jpg)

![2020-11-08-jtb4EU](https://image.ldbmcs.com/2020-11-08-jtb4EU.jpg)
...
![2020-11-08-DTP8nH](https://image.ldbmcs.com/2020-11-08-DTP8nH.jpg)

![2020-11-08-hXr0sI](https://image.ldbmcs.com/2020-11-08-hXr0sI.jpg)
