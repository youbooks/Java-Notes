...
> 转载：[四种常见的 POST 提交数据方式](https://imququ.com/post/four-ways-to-post-data-in-http.html)
...
这种方案，可以方便的提交复杂的结构化数据，特别适合 RESTful 的接口。各大抓包工具如 Chrome 自带的开发者工具、Firebug、Fiddler，都以树形结构展示 JSON 数据，非常友好。但也有些服务端语言还没有支持这种方式，例如 php 就无法通过 $\_POST 对象从上面的请求中获得内容。这时候，需要自己动手处理下：a在请求头中 Content-Type 为 application/json 时，从 `php://input` 里获得原始输入流，再 `json_decode` 成对象。一些 php 框架已经开始这么做了。
...
本文链接：[https://imququ.com/post/four-ways-to-post-data-in-http.html，\[参与评论](https://imququ.com/post/four-ways-to-post-data-in-http.html，[参与评论) »\]\([https://imququ.com/post/four-ways-to-post-data-in-http.html\#comments](https://imququ.com/post/four-ways-to-post-data-in-http.html#comments)\)
