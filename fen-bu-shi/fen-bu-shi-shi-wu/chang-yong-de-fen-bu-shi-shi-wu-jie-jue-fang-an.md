...
原文链接: [https://juejin.im/post/5aa3c7736fb9a028bb189bca](https://juejin.im/post/5aa3c7736fb9a028bb189bca)
...
1. 协调者节点向所有参与者节点**询问**是否可以执行提交操作\(vote\)，并开始等待各参与者节点的响应。
2. 参与者节点执行询问发起为止的所有事务操作，并将**Undo信息**和**Redo信息**写入日志。（注意：若成功这里其实每个参与者已经执行了事务操作）
...
2. 参与者节点利用之前写入的**Undo信息**执行回滚，并释放在整个事务期间内占用的资源。
...
1. 执行过程中，所有参与节点都是**事务阻塞型**的。当参与者占有公共资源时，其他第三方节点访问公共资源不得不处于阻塞状态。
2. 参与者发生故障。协调者需要给每个参与者额外指定**超时机制**，超时后整个事务失败。（没有多少容错机制）
...
* 在第一阶段和第二阶段中插入一个**准备阶段**。保证了在最后提交阶段之前各参与节点的状态是一致的。
...
1. 发送预提交请求 协调者向参与者发送PreCommit请求，并进入`Prepared阶段`。
2. 事务预提交 参与者接收到PreCommit请求后，会执行事务操作，并将undo和redo信息记录到事务日志中。
3. 响应反馈 如果参与者成功的执行了事务操作，则返回`ACK响应`，同时开始等待最终指令。
...
![](https://image.ldbmcs.com/2019-07-05-032927.jpg)
...
![](https://image.ldbmcs.com/2019-07-05-064149.jpg)
...
![](https://image.ldbmcs.com/2019-07-05-064232.jpg)
...
如果消息在投递过程中丢失，或消息的确认应答在返回途中丢失，那么消息中间件在等待确认应答超时之后就会重新投递，直到下游消费者返回消费成功响应为止。当然，一般消息中间件可以设置消息重试的次数和时间间隔，比如：当第一次投递失败后，每隔五分钟重试一次，一共重试3次。如果重试3次之后仍然投递失败，那么这条消息就需要**人工干预**。

![](https://image.ldbmcs.com/2019-07-05-064322.jpg)
...
![](https://image.ldbmcs.com/2019-07-05-064444.jpg)
...
如果重复投递和定期校对都不能解决问题，往往是因为下游系统出现了严重的错误，此时就需要**人工干预**。
...
![](https://image.ldbmcs.com/2019-07-05-064718.jpg)
...
* [大规模SOA系统中的分布事务处理\_程立](https://link.juejin.im/?target=http%3A%2F%2Fwww.infoq.com%2Fcn%2Finterviews%2Fsoa-chengli)
* [Life beyond Distributed Transactions: an Apostate’s Opinion](https://link.juejin.im/?target=https%3A%2F%2Fcs.brown.edu%2Fcourses%2Fcs227%2Farchives%2F2012%2Fpapers%2Fweaker%2Fcidr07p15.pdf)
* [关于如何实现一个TCC分布式事务框架的一点思考](https://link.juejin.im/?target=http%3A%2F%2Fwww.bytesoft.org%2Ftcc-intro%2F)
* [How can a requestor ensure a consistent outcome across multiple, independent providers](https://link.juejin.im/?target=http%3A%2F%2Fwww.enterpriseintegrationpatterns.com%2Fpatterns%2Fconversation%2FTryConfirmCancel.html)
* [关于分布式事务、两阶段提交协议、三阶提交协议](https://link.juejin.im/?target=http%3A%2F%2Fwww.hollischuang.com%2Farchives%2F681%23rd%3Fsukey%3D3997c0719f1515205acb269da14295ad50b0186483fbd0a402a566f45b33525978b375ccc44dba3e85c4d645a320ba47)
* [Three-phase commit protocol](https://link.juejin.im/?target=https%3A%2F%2Fen.wikipedia.org%2Fwiki%2FThree-phase_commit_protocol_ei.cs.vt.edu%2F~cs5204%2Ffall99%2FdistributedDBMS%2Fsreenu%2F3pc.html)
