...
原文链接：[分布式事务的实现原理](https://draveness.me/distributed-transaction-principle/)
...
作为悲观并发控制机制，锁使用在更新资源之前对资源进行锁定的方式保证多个数据库的会话同时修改某一行记录时不会出现脱离预期的行为\(悲观锁\)，而时间戳这种方式在每次提交时对资源是否被改变进行检查\(乐观锁\)。
