...
> 转载：[为什么我的MySQL会“抖”一下？](https://time.geekbang.org/column/article/71806)
...
![2021-03-05-RNVW5U](https://image.ldbmcs.com/2021-03-05-RNVW5U.jpg)

**图 1 “孔乙己赊账”更新和 flush 过程**

回到文章开头的问题，你不难想象，平时执行很快的更新操作，其实就是在写内存和日志，而 MySQL 偶尔“抖”一下的那个瞬间，可能就是在**刷脏页（flush）**。
...
- 第一种场景是，粉板满了，记不下了。这时候如果再有人来赊账，掌柜就只得放下手里的活儿，将粉板上的记录擦掉一些，留出空位以便继续记账。当然在擦掉之前，他必须先将正确的账目记录到账本中才行。
  ...
  ![2021-03-05-Ia7rIc](https://image.ldbmcs.com/2021-03-05-Ia7rIc.jpg)
...
- 第二种场景是，这一天生意太好，要记住的事情太多，掌柜发现自己快记不住了，赶紧找出账本把孔乙己这笔账先加进去。
  ...
  >
  > 加速写就是当需要修改一个数据页的时候，先将这个页在缓冲池中(内存)进行修改，记下相关的 redo log，这个页的修改就算已经完成了。至于这个被修改的页什么时候真正刷新到磁盘，这个是 buffer pool 后台刷新线程来完成的。
  >
  > 在实现上面两个功能的同时，需要考虑客观条件的限制，因为机器的**内存大小**是有限的。通常情况下，当数据库的数据量比较大的时候，缓存池并不能缓存所有的数据页，所以也就可能会出现，当需要访问的某个页时，该页却不在缓存池中的情况，这个时候就需要从磁盘中将这个页读出来，加载到缓存池，然后再去访问。这样就涉及到随机的物理 IO，也就增加了操作页所消耗的时间。
...
![2021-03-05-oWRqGY](https://image.ldbmcs.com/2021-03-05-oWRqGY.jpg)
