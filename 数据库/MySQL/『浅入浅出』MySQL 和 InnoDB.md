...
> 原文链接：[『浅入浅出』MySQL 和 InnoDB](https://draveness.me/mysql-innodb/)
...
![2021-02-09-dOnCYi](https://image.ldbmcs.com/2021-02-09-dOnCYi.jpg)
...
在 MySQL 中，实例和数据库往往都是一一对应的，而我们也无法直接操作数据库，而是要通过数据库实例来操作数据库文件，可以理解为**数据库实例是数据库为上层提供的一个专门用于操作的接口**。

![2021-02-09-icNb15](https://image.ldbmcs.com/2021-02-09-icNb15.jpg)
...
![2021-02-09-ZCf4rv](https://image.ldbmcs.com/2021-02-09-ZCf4rv.jpg)

**最上层**用于连接、线程处理的部分并不是 MySQL 『发明』的，很多服务都有类似的组成部分；**第二层**中包含了大多数 MySQL 的核心服务，包括了对 SQL 的解析、分析、优化和缓存等功能，存储过程、触发器和视图都是在这里实现的；而**第三层**就是 MySQL 中真正负责数据的存储和提取的**存储引擎**，例如：[InnoDB](https://en.wikipedia.org/wiki/InnoDB)、[MyISAM](https://en.wikipedia.org/wiki/MyISAM) 等，文中对存储引擎的介绍都是对 InnoDB 实现的分析。

## 2. 数据的存储

在整个数据库体系结构中，我们可以使用不同的**存储引擎**来存储数据，而绝大多数存储引擎都以二进制的形式存储数据；这一节会介绍 InnoDB 中对数据是如何存储的。

在 InnoDB 存储引擎中，所有的数据都被**逻辑地**存放在表空间中，**表空间（tablespace）**是存储引擎中最高的存储逻辑单位，在表空间的下面又包括**段（segment）、区（extent）、页（page）**：

![2021-02-09-OhBjuI](https://image.ldbmcs.com/2021-02-09-OhBjuI.jpg)

同一个数据库实例的所有表空间都有相同的页大小；默认情况下，表空间中的页大小都为 16KB，当然也可以通过改变 `innodb_page_size` 选项对默认大小进行修改，需要注意的是不同的页大小最终也会导致区大小的不同：

![2021-02-09-km6evK](https://image.ldbmcs.com/2021-02-09-km6evK.jpg)
...
![2021-02-09-5ikufu](https://image.ldbmcs.com/2021-02-09-5ikufu.jpg)
...
![2021-02-09-g4o4RL](https://image.ldbmcs.com/2021-02-09-g4o4RL.jpg)
...
![2021-02-09-f3ONU9](https://image.ldbmcs.com/2021-02-09-f3ONU9.jpg)

> Antelope 是 InnoDB 最开始支持的文件格式，它包含两种行格式 Compact 和 Redundant，它最开始并没有名字；Antelope 的名字是在新的文件格式 Barracuda 出现后才起的，Barracuda 的出现引入了两种新的行格式 Compressed 和 Dynamic；InnoDB 对于文件格式都会向前兼容，而官方文档中也对之后会出现的新文件格式预先定义好了名字：Cheetah、Dragon、Elk 等等。

两种行记录格式 Compact 和 Redundant 在磁盘上按照以下方式存储：

![2021-02-09-FalZiY](https://image.ldbmcs.com/2021-02-09-FalZiY.jpg)
...
![2021-02-09-4V6McK](https://image.ldbmcs.com/2021-02-09-4V6McK.jpg)

但是当我们使用新的行记录格式 Compressed 或者 Dynamic 时都只会在行记录中保存 20 个字节的指针，实际的数据都会存放在溢出页面中。

![2021-02-09-u2d2Qs](https://image.ldbmcs.com/2021-02-09-u2d2Qs.jpg)
...
![2021-02-09-VKvLP5](https://image.ldbmcs.com/2021-02-09-VKvLP5.jpg)
...
![2021-02-09-qBpgmj](https://image.ldbmcs.com/2021-02-09-qBpgmj.jpg)
...
![2021-02-09-E0gcB2](https://image.ldbmcs.com/2021-02-09-E0gcB2.jpg)
...
![2021-02-09-A2tnzR](https://image.ldbmcs.com/2021-02-09-A2tnzR.jpg)
...
![2021-02-09-2dQS4c](https://image.ldbmcs.com/2021-02-09-2dQS4c.jpg)

如果在表 `users` 中存在一个辅助索引 `(first_name, age)`，那么它构成的 B+ 树大致就是上图这样，按照 `(first_name, age)` 的字母顺序对表中的数据进行排序，当查找到主键时，再通过聚集索引获取到整条行记录。

![2021-02-09-KDx5d9](https://image.ldbmcs.com/2021-02-09-KDx5d9.jpg)
...
![2021-02-09-j5VZAM](https://image.ldbmcs.com/2021-02-09-j5VZAM.jpg)
...
![2021-02-09-uFu6sL](https://image.ldbmcs.com/2021-02-09-uFu6sL.jpg)
...
![2021-02-09-Q6geyX](https://image.ldbmcs.com/2021-02-09-Q6geyX.jpg)
...
![2021-02-09-ldM34W](https://image.ldbmcs.com/2021-02-09-ldM34W.jpg)
...
![2021-02-09-5O9sIz](https://image.ldbmcs.com/2021-02-09-5O9sIz.jpg)
...
![2021-02-09-ZpfOqv](https://image.ldbmcs.com/2021-02-09-ZpfOqv.jpg)
...
![2021-02-09-5XdtDW](https://image.ldbmcs.com/2021-02-09-5XdtDW.jpg)
...
![2021-02-09-9eOGTn](https://image.ldbmcs.com/2021-02-09-9eOGTn.jpg)

这种现象在数据库中就被称作幻读，虽然我们使用查询语句得到了一个空的集合，但是插入数据时却得到了错误，好像之前的查询是幻觉一样。

在标准的事务隔离级别中，幻读是由更高的隔离级别 `SERIALIZABLE` 解决的，但是它也可以通过 MySQL 提供的 Next-Key 锁解决：

![2021-02-09-ZKhXIr](https://image.ldbmcs.com/2021-02-09-ZKhXIr.jpg)
