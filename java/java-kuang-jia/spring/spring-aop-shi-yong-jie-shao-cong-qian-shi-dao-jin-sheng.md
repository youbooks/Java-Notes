...
原文链接: [https://javadoop.com/post/spring-aop-intro](https://javadoop.com/post/spring-aop-intro)
...
* AspectJ 出身也是名门，来自于 Eclipse 基金会，link：[https://www.eclipse.org/aspectj](https://www.eclipse.org/aspectj)
...
* ~~很快我会专门写一篇文章介绍 AspectJ 的使用，以及怎么在 Spring 应用中使用 AspectJ。~~
  ...
  > 已成文：[https://www.javadoop.com/post/aspectj](https://www.javadoop.com/post/aspectj)
...
现在可以开始配置了，我们配置一个名为 **spring\_1\_2.xml** 的文件：
...
从结果可以看出，create _方法使用了 logArgsAdvisor 进行传参输出，query_ 方法使用了 logResultAdvisor 进行了返回结果输出。
...
* @annotation：方法上具有特定的注解，如 @Subscribe 用于订阅特定的事件。
  ...
  > 如 @Pointcut\("execution\( __.\*\(..\)\) && @annotation\(com.javadoop.annotation.Subscribe\)"\)
...
《[AspectJ 使用介绍](https://github.com/ldbmcs/Developer-Notes/tree/ddace1117044e37a0a3d84dc8f5d22a014f3ffd8/post/aspectj/README.md)》介绍了 AspectJ 的 3 种用法，感兴趣的读者可以去看一看，那篇文章稍微短一些。
